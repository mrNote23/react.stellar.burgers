...
class _Api {
  ...
  private _checkSuccess = (response: TResponse) => {
    return response.success ? Promise.resolve(response) : Promise.reject(`Response not success ${response}`)
  }

  private _request = (url: string, options = {}) => {
    return fetch(`${this._baseUrl}${url}`, options).then(this._checkResponse).then(this._checkSuccess)
  }
