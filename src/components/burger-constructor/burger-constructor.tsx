...
import { burgerAddIngredient, burgerClearIngredients, burgerRemoveIngredient, burgerSwapIngredients } from "../../services/reducers/burger"
import { ingredientCounterDec, ingredientCounterInc, ingredientsCountersReset } from "../../services/reducers/ingredients"
...
const BurgerConstructor = () => {
  ...
  const onResize = () => {
    ...
    ;(scrolledWindow.current as HTMLElement).style.maxHeight = `${maxHeight < 96 ? 96 : maxHeight}px`
  }
  ...
  const burgerEmpty = useMemo(() => !(burger.bun !== null || burger.filling.length > 0), [burger])

  const burgerPrice = useMemo(() => (!burgerEmpty ? burger.filling.reduce((acc, item) => acc + item.price, burger.bun ? burger.bun.price * 2 : 0) : 0), [burger, burgerEmpty])
    ...
    <section className={styles.section} ref={dropIngredient}>
      <div className={styles.list}>
        ...
        <div className={styles.scrolled} ref={scrolledWindow}>
          {burger.filling.map((item, index) => (
            <BurgerConstructorItem ingredient={item} onDelete={deleteIngredient} index={index} key={index} onSwap={swapIngredients} />
          ))}
        </div>
        {burger.bun && (
          <div className={styles.item}>
      ...
      {!burgerEmpty && (
        <div className={styles.footer}>
          ...
          <Button htmlType='button' type='primary' size='medium' extraClass='ml-10' onClick={createOrder} disabled={loading}>
            Оформить заказ
          </Button>
        </div>
      )}
