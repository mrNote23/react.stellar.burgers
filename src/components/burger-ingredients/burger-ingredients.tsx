...
const BurgerIngredients: FC<TBurgerIngredientsProps> = ({ data }) => {
  ...
  useEffect(() => {
    const targets = [bunTarget.current, mainTarget.current, sauceTarget.current]
    ...
    targets.forEach((target) => observer.observe(target as HTMLParagraphElement))
  }, [])

  const buns = useMemo(() => data.filter((elm: TIngredient) => elm.type === BUN_TYPE), [data])
  const sauces = useMemo(() => data.filter((elm: TIngredient) => elm.type === SAUCE_TYPE), [data])
  const mains = useMemo(() => data.filter((elm) => elm.type === MAIN_TYPE), [data])
    ...
    <section className={styles.section}>
      ...
      <div className={`${styles.ingredients} mt-5`} ref={scrollWindow}>
        ...
        {buns.map((item: TIngredient) => (
          <BurgerIngredientsItem key={item._id} ingredient={item} onClick={() => showIngredientDetails(item)} />
        ))}
      ...
      <Modal visible={showModal} setVisible={setShowModal} title='Детали ингредиента'>
        {currentIngredient.current && <IngredientDetails ingredient={currentIngredient.current} />}
      </Modal>
    </section>
  )
}
