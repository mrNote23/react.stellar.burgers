...
import { closeAllData, onConnectAllData, onLoadAllData } from "./feed-reducer";
...
describe("Feed Reducer Test", () => {
  it("On connected", () => {
    store.dispatch(onConnectAllData(true));
    const result = store.getState().feed;
    expect(result.connected).toBe(true);
  });

  it("On disconnected", () => {
    store.dispatch(closeAllData());
    const result = store.getState().feed;
    expect(result.connected).toBe(false);
  });

  it("On load data", () => {
    store.dispatch(onLoadAllData(mockData));
    const result = store.getState().feed;
